...
>>>>>>> 82dd936 (Ejemplo de data binding a una colecci√≥n de objetos)
...
>>>>>>> 82dd936 (Ejemplo de data binding a una colecci√≥n de objetos)
	...
	private void CargarDatos()
	{
		...
		_origenes.Add(new OrigenDePaquete
		{
			...
			//otra forma (@ = cadena verbativa) Origen = @"C:\Program Files(x86)\Microsoft SDKs\NuGetPAckages",
	...
	private void OnDeleteButtonClicked(object sender, EventArgs e)
	{
		...
		if (seleccionado != null)
		{
			...
			OrigenDePaquete? nuevoSeleccionado;  //El signo de interrogaciÛn es para declarar que acepta nulos
			if (_origenes.Count > 1)
			{
				//Hay m·s de un elemento
				if (indice < _origenes.Count - 1)
				{
					//El elemento seleccionado no es el ˙ltlimo
					nuevoSeleccionado = _origenes[indice + 1];
				} 
				else
				{
					//El elemento seleccionado es el ˙ltimo
					nuevoSeleccionado = _origenes[indice - 1];
				}
			} 
			else
